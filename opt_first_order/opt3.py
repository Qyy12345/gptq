...
# Version 2_3: Adaptive Channel-wise Wiener Shrinkage (James-Stein)
@torch.enable_grad()
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    Version 2_3 upgrade:
      - Accumulates BOTH first moment (sum) and second moment (sum-of-squares)
        of per-channel gradients in the backward hook.
      - Computes a Wiener / James-Stein shrinkage factor per output channel:
            eta_i = mu_i^2 / (mu_i^2 + lambda_i / N_total)
        so that channels with low SNR are automatically regularised.
      - Final alpha: alpha_final = eta * (1 / lambda).
    ...
    """
    print('TS-PTQ (v2_3 Wiener): Pre-computing gradients ...')
    model.to(dev)
    ...
    grad_y_sq_accum  = {n: 0.0 for n in all_linear_layers}  # Second moment accumulator
    grad_y_sum_accum = {n: 0.0 for n in all_linear_layers}  # <<< NEW: First moment accumulator
    ...
    def make_backward_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            # Accumulate Second Moment (lambda)
            grad_y_sq_accum[name] = grad_y_sq_accum[name] + grad_y_per_token.pow(2).sum(0).cpu()

            # <<< NEW: Accumulate First Moment (mu) >>>
            grad_y_sum_accum[name] = grad_y_sum_accum[name] + grad_y_per_token.sum(0).cpu()
    ...
    # Wiener Shrinkage computation for adaptive alpha
    result_g_global  = {}
    result_alpha_vec = {}
    ...
    for name in all_linear_layers:
        ...
        # Strict token-level mathematical expectation for g_global
        result_g_global[name] = g_global_accum[name] / n

        # ===== Adaptive Channel-wise Wiener Shrinkage (v2_3) =====
        # 1. Base Second Moment (lambda) & Base Alpha
        lambda_vec = grad_y_sq_accum[name] / n
        # Adaptive Tikhonov floor: use layer median to prevent alpha explosion
        # For normal channels (lambda >> gamma), alpha is essentially unchanged.
        # For pathological channels (lambda << gamma), alpha is capped at 1/gamma.
        layer_var_median = torch.median(lambda_vec)
        gamma_dynamic = 0.01 * layer_var_median + 1e-12
        alpha_base = 1.0 / (lambda_vec + gamma_dynamic)

        # 2. First Moment (mu)
        mu_vec = grad_y_sum_accum[name] / n

        # 3. Wiener Shrinkage Factor (eta)
        #    eta_i = mu_i^2 / (mu_i^2 + Var(g) / n), Var(g) = E[g^2] - E[g]^2
        mu_sq = mu_vec.pow(2)
        variance_of_mean = (lambda_vec - mu_sq) / n
        eta_vec = mu_sq / (mu_sq + variance_of_mean + 1e-12)  # 1e-12 prevents div-by-zero

        # 4. Final Adaptive Alpha (Broadcastable)
        alpha_final = eta_vec * alpha_base
    ...
    print(f'TS-PTQ (v2_3 Wiener): Pre-computed gradients for {len(result_g_global)} layers.')
...
if __name__ == '__main__':
    ...
    if args.wbits < 16 and not args.nearest:
        # TS-PTQ v2_3 Step 1: Pre-compute g_global and Wiener-adaptive alpha_vec
        g_global_dict, alpha_vec_dict = precompute_tsptq_gradients(model, dataloader, DEV)

        # TS-PTQ v2_3 Step 2: Run quantization with pre-computed shift data
        tick = time.time()
