...
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    grad_y_sq_accum  = {n: 0.0 for n in all_linear_layers} # For lambda (variance)
    grad_y_sum_accum = {n: 0.0 for n in all_linear_layers} # For mu (mean) - NEW for SNR
    ...
    def make_backward_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            # Accumulate expected squared gradient over tokens for lambda_vec
            grad_y_sq_accum[name] = grad_y_sq_accum[name] + grad_y_per_token.pow(2).sum(0).cpu()
            
            # Accumulate expected gradient sum over tokens for mu_vec (SNR calculation)
    ...
    for name in all_linear_layers:
        ...
        # ================= TS-PTQ: SNR-Aware Shrinkage =================
        # 1. 计算均值 (mu) 和 二阶矩/方差 (lambda)
        lambda_vec = grad_y_sq_accum[name] / n
        mu_vec = grad_y_sum_accum[name] / n

        # 2. 计算纯牛顿步长倒数 (保持原生 Machine Epsilon 1e-8)
        alpha_raw = 1.0 / (lambda_vec + 1e-8) 

        # 3. 计算通道级自适应收缩因子 (Wiener Shrinkage / eta_i)
        mu_sq = mu_vec.pow(2)
        variance_of_mean = lambda_vec / n
        # 如果均值极小且样本方差极大，eta_vec 会自动趋近于 0，完美对抗过拟合
        eta_vec = mu_sq / (mu_sq + variance_of_mean + 1e-12)
