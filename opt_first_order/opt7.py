...
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    """
    print('TS-PTQ: Pre-computing gradients (Plan C: Deadzone Thresholding) ...')
    model.to(dev)
    ...
    # === 方案 C 核心：信噪比死区软截断 ===
    # 使用自适应阈值（基于梯度幅度的中位数）和 sigmoid 软过渡替代硬编码阈值
    deadzone_ratio = 1e-2  # 阈值 = 1% × median(|g|)
    # ===============================================
    ...
    for name in all_linear_layers:
        ...
        g_global_layer = g_global_accum[name] / n
        result_g_global[name] = g_global_layer
        ...
        # === 方案 C 核心：自适应死区逻辑 ===
        # 计算每个输出通道 (即 g_global_layer 的每一行) 的梯度绝对值均值
        # Shape: [out_features]
        g_magnitude = torch.mean(torch.abs(g_global_layer), dim=1)

        # 自适应阈值：基于该层梯度幅度的中位数，避免硬编码尺度问题
        median_mag = torch.median(g_magnitude)
        threshold = deadzone_ratio * median_mag

        # 软阈值替代硬截断：sigmoid 提供平滑过渡，避免断崖式行为
        sharpness = 100.0 / (threshold + 1e-12)
        mask = torch.sigmoid(sharpness * (g_magnitude - threshold))

        # 软截断：梯度极小通道的 alpha_final 将平滑趋近 0.0
        alpha_final = alpha_raw * mask
        # ==================================
