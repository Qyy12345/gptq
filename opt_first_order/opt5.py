...
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    """
    print('TS-PTQ: Pre-computing gradients (Plan A: Robust Median Estimation) ...')
    model.to(dev)
    ...
    # === 方案 A 核心：不再使用全局无脑累加，而是记录每个 Batch 的方差 ===
    grad_y_sq_history = {n: [] for n in all_linear_layers}
    # =====================================================================
    ...
    def make_backward_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            # 1. Accumulate expected gradient (sum over tokens) for direction
            ...
            # === 方案 A 核心：计算当前 Batch 的梯度方差，并存入历史记录 ===
            # 这里取当前 Batch 内部的 Mean，得到代表这个 Batch 的方差向量
            batch_grad_y_sq = grad_y_per_token.pow(2).mean(dim=0).cpu() # Shape: [out_features]
            grad_y_sq_history[name].append(batch_grad_y_sq)
            # ===============================================================
    ...
    for name in all_linear_layers:
        ...
        # 1. g_global 依然使用 Mean，保证一阶梯度的方向无偏
        result_g_global[name] = g_global_accum[name] / n

        # === 方案 A 核心：使用中位数 (Median) 替代均值 (Mean) 提取真实方差 ===
        # 把 128 个 Batch 的方差堆叠起来 -> Shape: [num_batches, out_features]
        stacked_sq = torch.stack(grad_y_sq_history[name], dim=0)

        # 沿着 Batch 维度求中位数，彻底无视极端异常 Batch！-> Shape: [out_features]
        lambda_vec_median = torch.median(stacked_sq, dim=0).values

        # Fix: 用全局 mean 归一化 median，消除 median < mean 导致 alpha 偏大的系统性偏差
        global_mean_sq = grad_y_sq_accum[name] / n
        correction = global_mean_sq / (lambda_vec_median.mean() + 1e-12)
        lambda_vec_corrected = lambda_vec_median * correction

        # 使用归一化后的鲁棒 Median 代入公式计算 alpha
        result_alpha_vec[name] = (1.0 / (lambda_vec_corrected + 1e-8)).unsqueeze(1)
        # ======================================================================
