...
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    """
    print('TS-PTQ: Pre-computing gradients ...')
    model.to(dev)
    ...
    # === TS-PTQ: 方案二 (方向一致性) 的新增累加器 ===
    grad_y_sign_accum = {n: 0.0 for n in all_linear_layers} # 记录符号和
    batch_count       = {n: 0 for n in all_linear_layers}   # 记录 Batch 数量
    # ===============================================
    ...
    def make_backward_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            # === TS-PTQ: 方案二 (方向一致性) 核心逻辑 ===
            # 计算当前这个 Batch (整句话) 的平均梯度方向
            batch_grad_y = grad_y_per_token.mean(dim=0).cpu() # Shape: [out_features]
            # 累加符号 (取值为 +1, 0, 或 -1)
            grad_y_sign_accum[name] = grad_y_sign_accum[name] + torch.sign(batch_grad_y)
            batch_count[name] = batch_count[name] + 1
            # ============================================
    ...
    for name in all_linear_layers:
        ...
        # Fix: strict token-level mathematical expectation
        result_g_global[name] = g_global_accum[name] / n

        # 1. 计算理论上的高方差牛顿步倒数 (保持 Machine Epsilon)
        grad_y_mean_sq = grad_y_sq_accum[name] / n
        alpha_raw = 1.0 / (grad_y_mean_sq + 1e-8)  # Shape: [out_features]

        # === TS-PTQ: 方案二 (方向一致性) 收缩计算 ===
        # 计算 128 个 Batch 中的方向一致性比例 (-1.0 到 1.0)
        sign_mean = grad_y_sign_accum[name] / b
        
        # 提取绝对一致性作为置信度 [0.0, 1.0]
        # 如果一半 Batch 要变大，一半要变小，eta_vec 趋近于 0 (屏蔽噪声)
        # 如果所有 Batch 方向相同，eta_vec 趋近于 1.0 (全速前进)
        eta_vec = torch.abs(sign_mean)
        
        # 可选：你可以对 eta_vec 平方 (eta_vec = eta_vec.pow(2))，以更激进地惩罚那些一致性不高的噪声通道
