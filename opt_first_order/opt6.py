...
def precompute_tsptq_gradients(model, dataloader, dev):
    ...
    """
    print('TS-PTQ: Pre-computing gradients (Plan B: Inter-batch Variance Penalty) ...')
    model.to(dev)
    ...
    # === 方案 B 核心：记录 Batch 级别的均值和平方均值，用于求方差 ===
    batch_g_sum    = {n: 0.0 for n in all_linear_layers}
    batch_g_sq_sum = {n: 0.0 for n in all_linear_layers}
    ...
    def make_backward_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            # 1. Accumulate expected gradient (sum over tokens)
            ...
            # === 方案 B 核心：累加每个 Batch 的梯度和梯度平方 ===
            # 求出该 Batch 的梯度期望值 (Mean over tokens in this batch)
            batch_grad_y = grad_y_per_token.mean(dim=0).cpu() # Shape: [out_features]
            
            batch_g_sum[name]    = batch_g_sum[name] + batch_grad_y
            batch_g_sq_sum[name] = batch_g_sq_sum[name] + batch_grad_y.pow(2)
            batch_count[name]    += 1
            # ====================================================
    ...
    for name in all_linear_layers:
        ...
        # 2. 原始的物理牛顿步基线 alpha (基于全局 token 级平方期望)
        grad_y_mean_sq = grad_y_sq_accum[name] / n
        alpha_raw = 1.0 / (grad_y_mean_sq + 1e-8)  

        # === 方案 B 核心：计算批次间方差，并得出 SNR 惩罚系数 eta ===
        mu_g = batch_g_sum[name] / b          # E[g] across batches
        mu_g_sq = batch_g_sq_sum[name] / b    # E[g^2] across batches

        # 计算方差 Var(g) = E[g^2] - E[g]^2，加 clamp 防止浮点误差出现极小负数
        var_g = torch.clamp(mu_g_sq - mu_g.pow(2), min=0.0)

        # Fix: batch 间方差 Var_b(mean_t(g)) = Var(g)/T，但 Wiener 需要 Var(g)/N_total
        # 修正: 除以 batch 数 b，将 Var(g)/T 缩放到 Var(g)/(B*T) = Var(g)/N_total
        var_g_corrected = var_g / b

        # 计算自适应的方差惩罚因子 (0.0 ~ 1.0)
        # 如果 var_g_corrected 极大 (震荡噪声)，eta_vec 趋近于 0
        eta_vec = mu_g.pow(2) / (mu_g.pow(2) + var_g_corrected + 1e-12)
        
        # 用 eta 阻尼原始的 alpha
        alpha_final = eta_vec * alpha_raw
        # ==============================================================
