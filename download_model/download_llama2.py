#!/usr/bin/env python3
"""
使用镜像站下载 meta-llama/Llama-2-7b-hf
"""
import os
import sys
...
LOCAL_DIR = Path("/data2/user/quyiyang/gptq/models/llama-2-7b-hf")  # python llama.py models/llama-2-7b-hf c4 --wbits 3

os.chdir(LOCAL_DIR.parent)

print("=" * 50)
print("   下载 meta-llama/Llama-2-7b-hf")
...
def download_file(url, dest, timeout=1800):
    """下载单个文件"""
    if dest.exists():
    ...
    try:
        ...
        print(f"  [done] {dest.name} 完成 ({dest.stat().st_size / 1024 / 1024:.1f} MB)", flush=True)
        return True
    except Exception as e:
